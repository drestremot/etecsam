...
<div class="bg-etec-dark text-white py-14 border-b-4 border-etec-accent relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 relative z-10 flex items-center gap-6">
        ...
        <div>
            ...
            <p class="text-gray-300 text-sm">Vida Escolar, Matrículas, Diplomas e Documentação Acadêmica</p>
        </div>
    </div>
</div>
...
<div class="container mx-auto px-4 py-12 space-y-14">
    ...
    {{-- Perfil da Diretora / Responsável da Secretaria --}}
    @if($director)
            ...
            <div class="p-8 md:p-12 flex flex-col justify-center">
                <div class="flex items-center gap-2 mb-3">
                    <span class="inline-flex items-center gap-1.5 bg-etec-accent/20 text-etec-accent text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full">
                        ...
                        Diretoria de Secretaria Acadêmica
                    </span>
                </div>
    ...
    <div class="bg-white/50 dark:bg-white/5 rounded-2xl p-10 text-center border border-dashed border-gray-300 dark:border-white/10">
        ...
        <p class="text-gray-500 dark:text-gray-400">Informações da Secretaria em atualização.</p>
    </div>
    @endif

    {{-- Área do Aluno & Acesso Rápido --}}
    @if(isset($links) && count($links))
    <div>
        <h2 class="text-xl font-bold text-white mb-6 border-l-4 border-etec-accent pl-3 flex items-center gap-2">
            ...
            Área do Aluno & Sistemas Acadêmicos
        </h2>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            @foreach($links as $link)
            <a href="{{ $link['url'] }}" target="_blank"
               class="bg-etec-main border border-etec-dark/30 dark:border-white/10 p-5 rounded-2xl hover:shadow-lg hover:shadow-etec-dark/30 hover:border-etec-accent hover:-translate-y-1 transition group text-center flex flex-col items-center gap-2.5 h-full">
                <div class="text-2xl bg-white/10 text-etec-accent w-14 h-14 rounded-2xl flex items-center justify-center group-hover:bg-etec-accent/30 transition font-bold">
                    {{ $link['icon'] }}
                </div>
                <div>
                    <strong class="block text-sm text-white group-hover:text-etec-accent transition leading-tight mb-0.5">{{ $link['name'] }}</strong>
    ...
    {{-- Equipe de Atendimento da Secretaria --}}
    @if($staff->isNotEmpty())
    <div>
        <h2 class="text-xl font-bold text-white mb-6 border-l-4 border-etec-accent pl-3">Equipe de Atendimento da Secretaria</h2>
            ...
            <a href="{{ route('academic-division') }}"
               class="group bg-etec-main rounded-xl border border-etec-dark/30 dark:border-white/10 shadow-sm p-6 flex items-center gap-5 hover:border-etec-accent hover:shadow-md hover:shadow-etec-dark/30 transition">
                ...
                <div class="flex-grow">
                    ...
                    <p class="text-sm text-green-100">Coordenação e cursos técnicos</p>
                </div>
                <svg class="w-5 h-5 text-green-200/70 group-hover:text-etec-accent transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                </svg>
    ...
    <div class="bg-white/50 dark:bg-white/5 rounded-2xl p-8 border border-gray-200 dark:border-white/10">
        <h2 class="text-xl font-bold text-white mb-6 flex items-center gap-2.5">
            ...
            Requerimentos e Documentos da Secretaria
        </h2>
