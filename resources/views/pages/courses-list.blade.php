    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="bg-gray-50 p-6 md:p-8 border-b border-gray-200 flex flex-col md:flex-row gap-6 items-center md:items-start">
            <div class="flex-grow text-center md:text-left">
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-etec-dark mb-1 leading-tight"
                   >
                    {{ $unit->name }}
                </h2>
                <p class="text-gray-500 text-sm flex items-center gap-1.5 justify-center md:justify-start">
                    <svg class="w-4 h-4 text-etec-medium flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"/></svg>
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4 flex-shrink-0">
                ...
                <div class="min-w-0">
                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-0.5">Coord. de Sala</span>
                    <strong class="text-sm text-etec-dark block leading-tight">{{ $unit->coordinator->name }}</strong>
                    <a href="mailto:{{ $unit->coordinator->email }}"
                       class="text-xs text-blue-600 hover:underline truncate block max-w-[180px]">
        ...
        <div class="p-6 md:p-8">
            <h3 class="font-bold text-gray-700 mb-6 flex items-center gap-2 text-sm uppercase tracking-wide">
            ...
            @if($unit->courses->isEmpty())
                <p class="text-gray-400 text-sm py-6 text-center">Nenhum curso cadastrado para esta unidade.</p>
            @else
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($unit->courses as $course)
                <div class="border border-gray-100 rounded-xl p-5 hover:border-etec-medium hover:shadow-md transition group bg-gray-50/50 hover:bg-white">
                    <div class="flex justify-between items-start mb-3">
                        <span class="bg-gray-200/60 text-gray-600 text-xs px-2.5 py-1 rounded-lg font-semibold">
                            {{ $course->type }}
                        </span>
                        <a href="{{ route('courses.show', $course->slug) }}"
                           class="w-7 h-7 bg-white rounded-lg flex items-center justify-center text-gray-400 border border-gray-200 hover:border-etec-accent hover:text-etec-accent transition shadow-sm">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                        </a>
                    </div>
                    <h4 class="text-base font-bold text-gray-800 mb-2 group-hover:text-etec-medium transition leading-snug">
                    ...
                    <a href="{{ route('courses.show', $course->slug) }}"
                       class="inline-flex items-center gap-1.5 text-sm font-bold text-etec-dark hover:text-etec-accent transition">
                        Ver detalhes
