    ...
    <div class="bg-etec-main rounded-2xl shadow-sm overflow-hidden border border-etec-dark/30 dark:border-white/10">
        <div class="flex flex-col md:flex-row">
            ...
            <div class="p-8 md:p-12 flex flex-col justify-center">
                ...
                @if($director->email || $director->phone || $director->lattes_url)
                <div class="bg-white/10 rounded-xl p-4 max-w-xl space-y-3">
                    @if($director->email)
                    <a href="mailto:{{ $director->email }}" class="flex items-center gap-2.5 text-sm text-green-100 hover:text-etec-accent transition">
                        <svg class="w-4 h-4 text-etec-accent flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @endif
                    @if($director->phone)
                    <a href="tel:{{ preg_replace('/\D/', '', $director->phone) }}" class="flex items-center gap-2.5 text-sm text-green-100 hover:text-etec-accent transition">
                        <svg class="w-4 h-4 text-etec-accent flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="{{ $director->lattes_url }}" target="_blank" class="flex items-center gap-2.5 text-sm text-green-100 hover:text-etec-accent transition">
                        <svg class="w-4 h-4 text-etec-accent flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if($director->bio)
                <div class="bg-white/10 rounded-xl p-4 text-left max-w-xl mt-3">
    ...
    <div class="bg-etec-main rounded-2xl shadow-sm border border-etec-dark/30 dark:border-white/10 p-6 flex flex-col sm:flex-row items-center sm:items-start gap-5">
        ...
        <div class="flex-grow text-center sm:text-left">
            ...
            <div x-data="{ open: false }" class="mt-3">
                ...
                <div x-show="open" x-cloak class="bg-white/10 rounded-lg p-3 mt-1.5 max-w-xl text-xs text-green-100 leading-relaxed [&_p]:mb-2 [&_p:last-child]:mb-0 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_a]:underline [&_a]:text-etec-accent">{!! $deputy->bio !!}</div>
            </div>
            @endif
        </div>
    </div>
        ...
        <div class="grid md:grid-cols-3 gap-5">
            <a href="{{ route('regional-supervision') }}"
               class="group bg-etec-main rounded-xl border border-etec-dark/30 dark:border-white/10 shadow-sm p-6 flex items-center gap-5 hover:border-etec-accent hover:shadow-md hover:shadow-etec-dark/30 transition">
                ...
                <div class="flex-grow">
                    <h3 class="font-bold text-white group-hover:text-etec-accent transition">Supervisão Regional</h3>
                    <p class="text-sm text-green-100">Supervisão de Ensino — PR01</p>
                </div>
