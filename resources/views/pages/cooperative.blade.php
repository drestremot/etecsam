...
{{-- Hero --}}
<x-page-header label="Escola Fazenda" title="Cooperativa Escola" subtitle="Gestores, Cooperados e Prestações de Contas">
    <x-slot:icon>
...
<div class="container mx-auto px-4 py-12">
    ...
    <div>
        <h2 class="text-lg sm:text-xl font-bold text-white mb-6 border-l-4 border-amber-400 pl-3">Gestores da Cooperativa</h2>
        ...
        @else
        <div class="bg-[#14284b] rounded-2xl p-8 text-center border border-dashed border-white/10">
            <p class="text-slate-400 text-xs">Gestores em atualização.</p>
        </div>
        @endif
    </div>
    ...
    {{-- Cooperados --}}
    <div>
        <h2 class="text-lg sm:text-xl font-bold text-white mb-6 border-l-4 border-amber-400 pl-3">Cooperados</h2>

        @if($members->isNotEmpty())
        <div class="bg-[#14284b] rounded-2xl shadow-sm border border-white/10 p-6">
            <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                @foreach($members as $member)
                <div class="flex items-center justify-between gap-2 bg-black/20 border border-white/5 rounded-xl px-4 py-2.5">
                    <span class="text-sm font-medium text-white truncate">{{ $member->name }}</span>
                    @if($member->registration_number)
                    <span class="text-xs text-amber-300 font-bold flex-shrink-0">#{{ $member->registration_number }}</span>
                    @endif
                </div>
                @endforeach
            </div>
        ...
        @else
        <div class="bg-[#14284b] rounded-2xl p-8 text-center border border-dashed border-white/10">
            <p class="text-slate-400 text-xs">Lista de cooperados em atualização.</p>
        </div>
        @endif
    </div>
