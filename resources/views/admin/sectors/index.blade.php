...
<div class="bg-white rounded-xl shadow-sm overflow-hidden" x-data="adminTable()">
    ...
    <div class="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
        ...
        <input x-model="q" @input="search()" type="text" placeholder="Buscar por nome, resumo..."
        ...
        <thead class="bg-gray-50">
            <tr>
                <th @click="sort('nome')" class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                    Setor <span class="ml-1 text-gray-400" x-text="icon('nome')"></span>
                </th>
            ...
            <tr class="hover:bg-gray-50 {{ !$sector->is_active ? 'opacity-60' : '' }}"
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center justify-end gap-1">
                        ...
                        <form action="{{ route('admin.sectors.destroy', $sector) }}" method="POST" class="inline"
                              onsubmit="return confirm('Remover \'{{ addslashes($sector->name) }}\'?')">
                            @csrf @method('DELETE')
