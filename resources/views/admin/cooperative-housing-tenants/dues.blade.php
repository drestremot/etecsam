...
@section('page-title', 'Mensalidades de ' . $cooperativeHousingTenant->name)
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <div>
                <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">{{ $cooperativeHousingTenant->name }}</h2>
                @if($cooperativeHousingTenant->room)
                <p class="text-xs text-gray-400 mt-0.5">Quarto/Vaga: {{ $cooperativeHousingTenant->room }}</p>
                @endif
            </div>
            <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $cooperativeHousingTenant->isUpToDate() ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                {{ $cooperativeHousingTenant->isUpToDate() ? 'Em dia' : 'Pendente' }}
            </span>
        </div>
        ...
        <div class="p-8 text-center text-gray-400 text-sm">
            ...
            <a href="{{ route('admin.cooperative-housing-fees.create') }}" class="text-indigo-600 hover:underline">Cadastrar valores</a>.
        </div>
        @else
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-right">
                        <form action="{{ route('admin.cooperative-housing-tenants.dues.toggle', [$cooperativeHousingTenant, $fee]) }}" method="POST">
                            @csrf @method('PATCH')
                            <button type="submit"
