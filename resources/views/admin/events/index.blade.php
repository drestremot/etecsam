        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 {{ !$event->is_active ? 'opacity-60' : '' }}"
                ...
                <td class="px-3 py-2.5 text-center">
                    <input type="checkbox" value="{{ $event->id }}" :checked="selected.map(String).includes('{{ $event->id }}')" @change="toggleItem('{{ $event->id }}', $event.target.checked)" data-bulk-item class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4 cursor-pointer">
                </td>
