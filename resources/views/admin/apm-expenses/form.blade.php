...
@section('page-title', $action === 'create' ? 'Nova Saída' : 'Editar Saída')

@section('content')
<div class="max-w-2xl">

    <div class="mb-4">
        <a href="{{ route('admin.apm-expenses.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-indigo-600 transition">
            ← Voltar para lista
        </a>
    </div>

    <form action="{{ $action === 'create' ? route('admin.apm-expenses.store') : route('admin.apm-expenses.update', $apmExpense) }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Dados da Saída</h2>
            </div>
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="description" value="{{ old('description', $apmExpense->description) }}" required
                           ...
                           placeholder="Ex: Compra de material para festa, Manutenção do parquinho">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="category" value="{{ old('category', $apmExpense->category) }}"
                               class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                               placeholder="Ex: Eventos, Manutenção, Material">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                            Valor (R$) <span class="text-red-500">*</span>
                        </label>
                        <input type="number" name="amount" step="0.01" min="0" value="{{ old('amount', $apmExpense->amount) }}" required
                               class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="due_date"
                               value="{{ old('due_date', $apmExpense->due_date ? $apmExpense->due_date->format('Y-m-d') : '') }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="paid_date"
                               value="{{ old('paid_date', $apmExpense->paid_date ? $apmExpense->paid_date->format('Y-m-d') : '') }}"
                               class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                              class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition resize-none">{{ old('notes', $apmExpense->notes) }}</textarea>
                </div>
