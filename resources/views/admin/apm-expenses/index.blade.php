...
<div class="min-h-screen bg-[#dfe1e5] px-3 sm:px-6 lg:px-8 py-5 sm:py-8 pb-20 sm:pb-8"
     x-data="{
         ...
         openEdit(exp, updateUrl) {
             this.editExpense = {
                 id: exp.id,
                 description: exp.description || '',
                 category: exp.category || '',
                 amount: exp.amount || '',
                 due_date: exp.due_date ? exp.due_date.substring(0, 10) : '',
                 paid_date: exp.paid_date ? exp.paid_date.substring(0, 10) : '',
                 notes: exp.notes || '',
                 updateUrl: updateUrl
             };
             this.editOpen = true;
    ...
    <div class="w-full max-w-[1850px] mx-auto space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 text-xs font-semibold text-gray-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-rose-700 font-bold">Despesas & Saídas</span>
                </div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-3">
                    <span class="flex items-center gap-2">
                        <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-rose-600 text-white shadow-sm">
                        ...
                        Despesas & Saídas da APM
                    </span>
                    <span class="rounded-xl bg-rose-100 border border-rose-200 px-2.5 py-0.5 text-xs font-semibold text-rose-800">
                        {{ count($apmExpenses) }} lançamentos
                    </span>
                </h1>
                <p class="text-xs text-gray-600 mt-0.5 font-normal">
                    Controle de contas a pagar, manutenções, compras de materiais e pagamentos da APM
                </p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <button @click="createOpen = !createOpen"
                        type="button"
                        class="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-rose-500 active:scale-95">
                    ...
                    <span x-text="createOpen ? 'Fechar Formulário' : '+ Nova Despesa'"></span>
                </button>
        ...
        {{-- Formulário Retrátil de Nova Saída --}}
        <div x-show="createOpen"
             ...
             class="rounded-2xl border border-rose-200 bg-white p-5 sm:p-6 shadow-sm"
             style="display: none;">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-gray-800">Lançar Nova Despesa / Saída da APM</h2>
                        <p class="text-[11px] text-gray-500 font-normal">Preencha a descrição, categoria, valor e prazo de pagamento</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.apm-expenses.store') }}" method="POST" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {{-- Descrição --}}
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">
                            Descrição da Despesa <span class="text-rose-500">*</span>
                        </label>
                        <input type="text" name="description" value="{{ old('description') }}" required
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 placeholder-gray-400 shadow-2xs focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"
                               placeholder="Ex: Aquisição de material pedagógico para laboratórios">
                    </div>
                    ...
                    <div>
                        ...
                        <input list="apm-expense-cats" type="text" name="category" value="{{ old('category') }}"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 placeholder-gray-400 shadow-2xs focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"
                               placeholder="Ex: Manutenção, Material, Eventos...">
                        <datalist id="apm-expense-cats">
                            <option value="Manutenção Predial">
                            <option value="Material de Escritório / Pedagógico">
                            <option value="Equipamentos e Tecnologia">
                            <option value="Eventos & Formaturas">
                            <option value="Serviços de Terceiros">
                            <option value="Taxas Bancárias">
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" value="{{ old('due_date', date('Y-m-d')) }}" required
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 shadow-2xs focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20">
                    </div>

                    {{-- Data de Pagamento Efetivo --}}
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">
                            Data de Pagamento (se já liquidado)
                        </label>
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">
                            Observações / Nº da Nota Fiscal
                        </label>
                        <input type="text" name="notes" value="{{ old('notes') }}"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 placeholder-gray-400 shadow-2xs focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"
                               placeholder="Nº NF, fornecedor ou detalhes do pagamento">
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 pt-2 border-t border-gray-100">
                    ...
                    <button type="submit" class="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-5 py-2 text-xs font-semibold text-white shadow-xs hover:bg-rose-500 active:scale-95 transition">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5 shadow-xs space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="relative flex-1 min-w-[240px] max-w-md">
                    ...
                    <input x-model="search"
                           ...
                           class="w-full pl-9 pr-8 py-2 rounded-xl border border-gray-300 bg-gray-50/50 text-xs text-gray-900 placeholder-gray-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <div class="flex items-center gap-1.5 bg-gray-100 p-1 rounded-xl">
                        ...
                        <button @click="statusFilter = 'pago'"
                                :class="statusFilter === 'pago' ? 'bg-emerald-600 text-white shadow-2xs font-bold' : 'text-gray-600 hover:text-gray-900 font-medium'"
                                class="px-2.5 py-1 rounded-lg text-xs transition">
                            Pagas
                        </button>
                    ...
                    @php
                        $uniqueExpenseCats = $apmExpenses->pluck('category')->filter()->unique()->sort()->values();
                    @endphp
                    @if($uniqueExpenseCats->count() > 1)
                    <select x-model="categoryFilter"
                            class="rounded-xl border border-gray-300 bg-white px-3 py-1.5 text-xs text-gray-700 shadow-2xs focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20">
                        <option value="all">Todas as Categorias ({{ $uniqueExpenseCats->count() }})</option>
                        @foreach($uniqueExpenseCats as $cat)
            ...
            <div x-show="selected.length > 0"
                 ...
                 x-transition:enter-end="opacity-100 translate-y-0"
                 class="flex flex-wrap items-center justify-between gap-3 bg-rose-50 border border-rose-200 rounded-xl px-4 py-2.5 text-xs text-rose-900"
                 style="display: none;">
                <div class="flex items-center gap-2 font-bold">
                    <span class="flex h-5 w-5 items-center justify-center rounded-full bg-rose-600 text-white text-[10px]" x-text="selected.length"></span>
                    <span>despesas selecionadas</span>
                </div>
                <div class="flex items-center gap-2">
                    <form action="{{ route('admin.apm-expenses.bulk-action') }}" method="POST" class="inline">
                        @csrf
                    ...
                    <form action="{{ route('admin.apm-expenses.bulk-action') }}" method="POST" class="inline"
                          onsubmit="return confirm('Tem certeza que deseja excluir as despesas selecionadas?')">
                        @csrf
                        <template x-for="id in selected" :key="id">
                            <input type="hidden" name="ids[]" :value="id">
                        </template>
                        ...
                        <tr>
                            <th class="w-10 px-3.5 py-3">
                                <input type="checkbox"
                                       ...
                                       class="h-4 w-4 rounded border-gray-300 text-rose-600 focus:ring-rose-500 transition cursor-pointer">
                            </th>
                            <th class="px-3.5 py-3">Descrição da Despesa</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse($apmExpenses as $expense)
                            @php
                                ...
                                $searchString = strtolower($expense->description . ' ' . ($expense->category ?? '') . ' ' . ($expense->notes ?? '') . ' ' . $status);
                            @endphp
                            <tr data-id="{{ $expense->id }}"
                                data-search="{{ $searchString }}"
                                data-status="{{ $statusKey }}"
                                data-cat="{{ strtolower($expense->category ?? '') }}"
                                class="hover:bg-rose-50/30 transition {{ $statusKey === 'pago' ? 'bg-emerald-50/10' : '' }}"
                                :class="selected.includes({{ $expense->id }}) ? 'bg-rose-50/50' : ''">
                                ...
                                <td class="px-3.5 py-3">
                                    ...
                                    @if($expense->notes)
                                        <div class="text-[11px] text-gray-500 mt-0.5">{{ $expense->notes }}</div>
                                    @endif
                                </td>
                                ...
                                {{-- Valor --}}
                                <td class="px-3.5 py-3 font-bold text-rose-700 text-sm">
                                    R$ {{ number_format($expense->amount, 2, ',', '.') }}
                                </td>

                                {{-- Vencimento / Pago --}}
                                <td class="px-3.5 py-3">
                                    <div class="text-gray-800 font-medium">{{ $expense->due_date->format('d/m/Y') }}</div>
                                    @if($expense->paid_date)
                                        <div class="text-[10px] text-emerald-700">Pago em {{ $expense->paid_date->format('d/m/Y') }}</div>
                                    @endif
                                </td>
                                ...
                                <td class="px-3.5 py-3 text-center">
                                    <form action="{{ route('admin.apm-expenses.mark-paid', $expense) }}" method="POST" class="inline">
                                        ...
                                        <button type="submit"
                                                ...
                                                {{ $status === 'Pago' ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' : ($status === 'Atrasado' ? 'bg-rose-100 text-rose-700 hover:bg-rose-200' : 'bg-amber-100 text-amber-800 hover:bg-amber-200') }}">
                                            <span class="h-1.5 w-1.5 rounded-full {{ $status === 'Pago' ? 'bg-emerald-600' : ($status === 'Atrasado' ? 'bg-rose-600' : 'bg-amber-500') }}"></span>
                                            <span>{{ $status }}</span>
                                        </button>
                                    </form>
                                </td>
                                ...
                                <td class="px-3.5 py-3 text-right">
                                    <div class="inline-flex items-center gap-1">
                                        <button @click="openEdit({{ json_encode($expense) }}, '{{ route('admin.apm-expenses.update', $expense) }}')"
                                                type="button"
                                                title="Editar Informações"
                                                class="flex h-7 w-7 items-center justify-center rounded-lg bg-rose-50 text-rose-800 hover:bg-rose-100 transition shadow-2xs">
                                        ...
                                        <form action="{{ route('admin.apm-expenses.destroy', $expense) }}" method="POST" class="inline"
                                              onsubmit="return confirm('Deseja realmente remover a despesa {{ addslashes($expense->description) }}?')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                    title="Excluir"
                            ...
                            <tr>
                                <td colspan="7" class="px-4 py-12 text-center text-gray-500">
                                    <div class="max-w-sm mx-auto space-y-2">
                                        <div class="flex h-12 w-12 mx-auto items-center justify-center rounded-2xl bg-rose-50 text-rose-700">
                                        ...
                                        <p class="text-sm font-bold text-gray-800">Nenhuma despesa cadastrada na APM</p>
                                        <p class="text-xs text-gray-500">Clique no botão acima para registrar um novo pagamento ou conta a pagar.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div @click.outside="editOpen = false"
            ...
            <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-gray-900">Editar Despesa da APM</h2>
                        <p class="text-[11px] text-gray-500 font-normal">Atualize os valores e a data de liquidação</p>
                    </div>
                </div>
            ...
            {{-- Modal Body --}}
            <form :action="editExpense.updateUrl" method="POST" class="p-5 space-y-4 overflow-y-auto flex-1">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="description" x-model="editExpense.description" required
                           class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input list="modal-apm-expense-cats" type="text" name="category" x-model="editExpense.category"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                        <datalist id="modal-apm-expense-cats">
                            <option value="Manutenção Predial">
                            <option value="Material de Escritório / Pedagógico">
                            <option value="Equipamentos e Tecnologia">
                            <option value="Eventos & Formaturas">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" name="amount" x-model="editExpense.amount" required
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" x-model="editExpense.due_date" required
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="paid_date" x-model="editExpense.paid_date"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="notes" x-model="editExpense.notes"
                           class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                </div>
