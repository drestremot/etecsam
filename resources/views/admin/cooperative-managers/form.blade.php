...
@section('page-title', $action === 'create' ? 'Novo Gestor' : 'Editar: ' . $cooperativeManager->name)

@section('content')
<div class="max-w-2xl">

    <div class="mb-4">
        <a href="{{ route('admin.cooperative-managers.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-indigo-600 transition">
            ← Voltar para lista
        </a>
    </div>

    <form action="{{ $action === 'create' ? route('admin.cooperative-managers.store') : route('admin.cooperative-managers.update', $cooperativeManager) }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Dados do Gestor</h2>
            </div>
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $cooperativeManager->name) }}" required
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                        Cargo na Cooperativa <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="role" value="{{ old('role', $cooperativeManager->role) }}" required
                           class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $cooperativeManager->email) }}"
                               class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" value="{{ old('phone', $cooperativeManager->phone) }}"
                               class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6">
                @if($action === 'edit' && $cooperativeManager->photo)
                    <div class="flex items-center gap-4 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <img src="{{ photo_url($cooperativeManager->photo) }}"
                             onerror="this.src='{{ avatar_url($cooperativeManager->name, '1a3a6e', 'fff', ['bold' => 'true', 'size' => 128]) }}'"
