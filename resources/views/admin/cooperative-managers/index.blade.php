...
@section('page-title', 'Gestores da Cooperativa')

@section('header-actions')
    <a href="{{ route('admin.cooperative-managers.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg">+ Novo</a>
@endsection
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden" x-data="adminTable()">
    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-50">
            <tr>
                ...
                <th @click="sort('cargo')" class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none">
                    Cargo <span class="ml-1 text-gray-400" x-text="icon('cargo')"></span>
                </th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 {{ !$manager->is_active ? 'opacity-60' : '' }}"
                data-row="{{ strtolower($manager->name . ' ' . $manager->role . ' ' . $manager->email) }}"
                data-active="{{ $manager->is_active ? '1' : '0' }}"
                data-nome="{{ strtolower($manager->name) }}"
                data-cargo="{{ strtolower($manager->role) }}"
                data-email="{{ strtolower($manager->email ?? '') }}"
                data-status="{{ $manager->is_active ? 'ativo' : 'inativo' }}">
                <td class="px-4 py-3 font-medium text-gray-800">
                    @php
                        $managerInitials = strtoupper(substr($manager->name, 0, 1));
                        if (preg_match('/\s+(\S)/u', $manager->name, $matches)) {
                            $managerInitials .= strtoupper($matches[1]);
                        }
                    @endphp
                ...
                <td class="px-4 py-3 text-gray-600">{{ $manager->role }}</td>
                <td class="px-4 py-3 text-gray-500">{{ $manager->email ?? '—' }}</td>
                <td class="px-4 py-3 text-gray-500">{{ $manager->phone ?? '—' }}</td>
                <td class="px-4 py-3">
                    <span class="px-2 py-1 rounded-full text-xs font-semibold {{ $manager->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center justify-end gap-1">
                        <form action="{{ route('admin.cooperative-managers.toggle', $manager) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                            <button type="submit" title="{{ $manager->is_active ? 'Desativar' : 'Ativar' }}"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg transition {{ $manager->is_active ? 'bg-green-50 text-green-600 hover:bg-green-100' : 'bg-gray-100 text-gray-400 hover:bg-gray-200' }}">
                        ...
                        <a href="{{ route('admin.cooperative-managers.edit', $manager) }}" title="Editar"
                        ...
                        <form action="{{ route('admin.cooperative-managers.destroy', $manager) }}" method="POST" class="inline"
                              onsubmit="return confirm('Remover {{ addslashes($manager->name) }}?')">
                            @csrf @method('DELETE')
    ...
    <div class="px-4 py-3">{{ $cooperativeManagers->links() }}</div>
</div>
