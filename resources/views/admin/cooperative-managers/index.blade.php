...
<div class="min-h-screen bg-[#dfe1e5] px-3 sm:px-6 lg:px-8 py-5 sm:py-8 pb-20 sm:pb-8"
    ...
    <div class="w-full max-w-[1850px] mx-auto space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 text-xs font-semibold text-gray-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-amber-700 font-bold">Gestores</span>
                </div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-3">
                    <span class="flex items-center gap-2">
                        ...
                        Gestores da Cooperativa Escola
                    </span>
                    <span class="rounded-xl bg-amber-100 border border-amber-200 px-2.5 py-0.5 text-xs font-semibold text-amber-800">
                        {{ count($cooperativeManagers) }} membros cadastrados
                    </span>
                </h1>
                <p class="text-xs text-gray-600 mt-0.5 font-normal">
                    Diretoria executiva, conselheiros fiscais e administração geral da Cooperativa Escola
                </p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('admin.cooperative-members.index') }}" class="inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-emerald-500">
                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    <span>Quadro de Cooperados</span>
                </a>

                <a href="{{ route('admin.cooperative-dashboard') }}" class="inline-flex items-center gap-1.5 rounded-xl bg-teal-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-teal-500">
        ...
        {{-- Card de Cadastro Rápido de Novo Gestor --}}
        <div x-show="createOpen"
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-gray-800">Cadastrar Novo Gestor da Cooperativa Escola</h2>
                        <p class="text-[11px] text-gray-500 font-normal">Preencha os dados do membro da diretoria executiva ou conselho de administração</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.cooperative-managers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">
                            Cargo na Cooperativa <span class="text-rose-500">*</span>
                        </label>
                        <div class="relative">
                            <input list="coop-roles-list" type="text" name="role" value="{{ old('role') }}" required
                                   class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 placeholder-gray-400 shadow-2xs focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                                   placeholder="Selecione ou digite o cargo...">
                            <datalist id="coop-roles-list">
                                <option value="Diretor Presidente">
                                <option value="Diretor Vice-Presidente">
                                <option value="Diretor Administrativo-Financeiro">
                                <option value="Diretor de Produção e Vendas">
                                <option value="Secretário">
                                <option value="Conselheiro Fiscal (Titular)">
                                <option value="Conselheiro Fiscal (Suplente)">
                                <option value="Gerente Operacional da Fazenda">
                                <option value="Membro do Conselho">
                            </datalist>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 placeholder-gray-400 shadow-2xs focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                               placeholder="carlos@etecsam.com.br">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" name="photo" accept="image/*"
                               class="w-full text-xs text-gray-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 transition">
                    </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5 shadow-xs space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @php
                        $uniqueCoopRoles = $cooperativeManagers->pluck('role')->unique()->sort()->values();
                    @endphp
                    @if($uniqueCoopRoles->count() > 1)
                    <select x-model="roleFilter"
                            class="rounded-xl border border-gray-300 bg-white px-3 py-1.5 text-xs text-gray-700 shadow-2xs focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20">
                        <option value="all">Todos os Cargos ({{ $uniqueCoopRoles->count() }})</option>
                        @foreach($uniqueCoopRoles as $role)
            ...
            <div x-show="selected.length > 0"
                ...
                <div class="flex items-center gap-2">
                    <form action="{{ route('admin.cooperative-managers.bulk-action') }}" method="POST" class="inline">
                        @csrf
                ...
                <table class="min-w-full divide-y divide-gray-200 text-left text-xs text-gray-700">
                    <thead class="bg-gray-50 text-gray-600 font-semibold uppercase tracking-wider text-[11px]">
                        <tr>
                            ...
                            <th class="px-3.5 py-3">Gestor / Membro</th>
                            <th class="px-3.5 py-3">Cargo na Cooperativa</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse($cooperativeManagers as $manager)
                            ...
                            <tr data-id="{{ $manager->id }}"
                                ...
                                class="hover:bg-amber-50/30 transition {{ !$manager->is_active ? 'bg-gray-50/70 text-gray-400' : '' }}"
                                :class="selected.includes({{ $manager->id }}) ? 'bg-amber-50/50' : ''">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="relative flex-shrink-0">
                                            @if($photoSrc)
                                                <img src="{{ $photoSrc }}" alt="{{ $manager->name }}"
                                                     class="h-10 w-10 rounded-full object-cover border-2 border-amber-100 shadow-2xs">
                                            @else
                                                <div class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-amber-500 to-amber-700 text-white font-bold text-xs shadow-2xs">
                                                    {{ $initials }}
                                                </div>
                                            @endif
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[11px] text-gray-500 flex items-center gap-2 mt-0.5">
                                                <span>Diretoria Cooperativa</span>
                                ...
                                {{-- Cargo --}}
                                <td class="px-3.5 py-3">
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-xs font-semibold bg-amber-50 text-amber-900 border border-amber-200">
                                        <svg class="w-3 h-3 text-amber-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                                        {{ $manager->role }}
                                    </span>
                                </td>
                                    ...
                                    <div class="space-y-0.5">
                                        @if($manager->email)
                                            <div class="flex items-center gap-1.5 text-gray-700">
                                                ...
                                                <a href="mailto:{{ $manager->email }}" class="hover:text-amber-700 hover:underline">{{ $manager->email }}</a>
                                            </div>
                                        @endif
                                        @if($manager->phone)
                                ...
                                <td class="px-3.5 py-3 text-center">
                                    <form action="{{ route('admin.cooperative-managers.toggle', $manager) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="submit"
                                ...
                                <td class="px-3.5 py-3 text-right">
                                    <div class="inline-flex items-center gap-1">
                                        <button @click="openEdit({{ json_encode($manager) }}, '{{ route('admin.cooperative-managers.update', $manager) }}', '{{ $photoSrc }}')"
                                                type="button"
                                        ...
                                        <form action="{{ route('admin.cooperative-managers.destroy', $manager) }}" method="POST" class="inline"
                                              onsubmit="return confirm('Deseja realmente remover o(a) gestor(a) {{ addslashes($manager->name) }} da Cooperativa?')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                    title="Excluir"
                            ...
                            <tr>
                                <td colspan="6" class="px-4 py-12 text-center text-gray-500">
                                    <div class="max-w-sm mx-auto space-y-2">
                                        ...
                                        <p class="text-sm font-bold text-gray-800">Nenhum gestor cadastrado na Cooperativa</p>
                                        <p class="text-xs text-gray-500">Clique no botão acima para adicionar diretores ou conselheiros.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div @click.outside="editOpen = false"
            ...
            <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-gray-900">Editar Gestor da Cooperativa</h2>
                        <p class="text-[11px] text-gray-500 font-normal">Atualize os dados e cargos</p>
                    </div>
                </div>
            ...
            <form :action="editManager.updateUrl" method="POST" enctype="multipart/form-data" class="p-5 space-y-4 overflow-y-auto flex-1">
                ...
                <div>
                    ...
                    <input list="modal-coop-roles-list" type="text" name="role" x-model="editManager.role" required
                           class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 shadow-2xs"
                           placeholder="Ex: Diretor Presidente, Diretor de Produção, etc.">
                    <datalist id="modal-coop-roles-list">
                        <option value="Diretor Presidente">
                        <option value="Diretor Vice-Presidente">
                        <option value="Diretor Administrativo-Financeiro">
                        <option value="Diretor de Produção e Vendas">
                ...
                <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-3.5 space-y-2">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1">
                            <input type="file" name="photo" accept="image/*"
                                   class="w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 transition">
