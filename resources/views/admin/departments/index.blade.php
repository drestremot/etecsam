...
<div class="bg-white rounded-xl shadow-sm overflow-hidden" x-data="adminTable()">
    ...
    <div class="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
        ...
        <input x-model="q" @input="search()" type="text" placeholder="Buscar por departamento, responsável..."
    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 {{ !$dept->is_active ? 'opacity-60' : '' }}"
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center justify-end gap-1">
                        <form action="{{ route('admin.departments.toggle', $dept) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                            <button type="submit" title="{{ $dept->is_active ? 'Desativar' : 'Ativar' }}"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg transition {{ $dept->is_active ? 'bg-green-50 text-green-600 hover:bg-green-100' : 'bg-gray-100 text-gray-400 hover:bg-gray-200' }}">
                                @if($dept->is_active)
                        ...
                        <a href="{{ route('admin.departments.edit', $dept) }}" title="Editar"
                        ...
                        <form action="{{ route('admin.departments.destroy', $dept) }}" method="POST" class="inline"
                              onsubmit="return confirm('Remover \'{{ addslashes($dept->name) }}\'?')">
                            @csrf @method('DELETE')
