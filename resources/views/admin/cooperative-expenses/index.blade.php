...
<div class="min-h-screen bg-[#dfe1e5] px-3 sm:px-6 lg:px-8 py-5 sm:py-8 pb-20 sm:pb-8"
    ...
    <div class="w-full max-w-[1850px] mx-auto space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 text-xs font-semibold text-gray-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-rose-700 font-bold">Despesas & Insumos</span>
                </div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-3">
                    <span class="flex items-center gap-2">
                        ...
                        Despesas & Custos da Cooperativa Escola
                    </span>
                    <span class="rounded-xl bg-rose-100 border border-rose-200 px-2.5 py-0.5 text-xs font-semibold text-rose-800">
                        R$ {{ number_format($cooperativeExpenses->sum('amount'), 2, ',', '.') }}
                    </span>
                </h1>
                <p class="text-xs text-gray-600 mt-0.5 font-normal">
                    Controle financeiro de insumos agrícolas, ração animal, sementes, defensivos e manutenção da fazenda
                </p>
            </div>
        ...
        {{-- Formulário Retrátil de Nova Despesa --}}
        <div x-show="createOpen"
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-gray-800">Lançar Nova Despesa / Custo da Cooperativa</h2>
                        <p class="text-[11px] text-gray-500 font-normal">Cadastre compras de ração, sementes, defensivos e contas da fazenda</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.cooperative-expenses.store') }}" method="POST" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {{-- Descrição --}}
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">
                            Descrição da Despesa / Fornecedor <span class="text-rose-500">*</span>
                        </label>
                        <input type="text" name="description" value="{{ old('description') }}" required
                               ...
                               placeholder="Ex: Compra de 20 sacos de ração para postura">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="category" value="{{ old('category', 'Insumos / Sementes') }}" list="expense-categories"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 placeholder-gray-400 shadow-2xs focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"
                               placeholder="Ex: Ração, Adubos...">
                        <datalist id="expense-categories">
                            <option value="Insumos & Sementes">
                            <option value="Nutrição / Ração Animal">
                            <option value="Medicamentos Veterinários">
                            <option value="Combustível & Máquinas">
                            <option value="Manutenção de Instalações">
                            <option value="Embalagens & Logística">
                        </datalist>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="notes" value="{{ old('notes') }}"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 placeholder-gray-400 shadow-2xs focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"
                               placeholder="NF nº, empresa fornecedora, forma de pagamento...">
                    </div>
                </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5 shadow-xs space-y-4">
            ...
            <div x-show="selected.length > 0"
                ...
                <div class="flex items-center gap-2">
                    <form action="{{ route('admin.cooperative-expenses.bulk-action') }}" method="POST" class="inline">
                        @csrf
            ...
            <div class="overflow-x-auto rounded-xl border border-gray-200">
                <table class="min-w-full divide-y divide-gray-200 text-left text-xs text-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse($cooperativeExpenses as $expense)
                            @php
                            ...
                            <tr data-id="{{ $expense->id }}"
                                ...
                                class="hover:bg-rose-50/30 transition"
                                ...
                                <td class="px-3.5 py-3 font-bold text-gray-900 text-sm">
                                    <div class="flex items-center gap-2">
                                        <span class="flex h-7 w-7 items-center justify-center rounded-lg bg-rose-100 text-rose-700 shrink-0">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/></svg>
                                        </span>
                                        <div>
                                            <span>{{ $expense->description }}</span>
                                            @if($expense->notes)
                                                <p class="text-[11px] text-gray-500 font-normal">{{ $expense->notes }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-3.5 py-3 text-center">
                                    <form action="{{ route('admin.cooperative-expenses.mark-paid', $expense) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="submit"
                                                ...
                                                {{ $status === 'Pago' ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' : ($status === 'Atrasado' ? 'bg-rose-100 text-rose-800 hover:bg-rose-200' : 'bg-amber-100 text-amber-800 hover:bg-amber-200') }}">
                                            <span class="h-1.5 w-1.5 rounded-full {{ $status === 'Pago' ? 'bg-emerald-600' : ($status === 'Atrasado' ? 'bg-rose-600' : 'bg-amber-600') }}"></span>
                                            <span>{{ $status }}</span>
                                        </button>
                                    </form>
                                </td>
                                ...
                                <td class="px-3.5 py-3 text-right">
                                    <div class="inline-flex items-center gap-1">
                                        <button @click="openEdit({{ json_encode($expense) }}, '{{ route('admin.cooperative-expenses.update', $expense) }}')"
                                                type="button"
                                        ...
                                        <form action="{{ route('admin.cooperative-expenses.destroy', $expense) }}" method="POST" class="inline"
                            ...
                            <tr>
                                <td colspan="7" class="px-4 py-12 text-center text-gray-500">
                                    <div class="max-w-sm mx-auto space-y-2">
                                        ...
                                        <p class="text-sm font-bold text-gray-800">Nenhuma despesa cadastrada na Cooperativa</p>
                                        <p class="text-xs text-gray-500">Clique no botão acima para registrar compras de insumos ou contas.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div @click.outside="editOpen = false"
            ...
            <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-gray-900">Editar Despesa da Cooperativa</h2>
                        <p class="text-[11px] text-gray-500 font-normal">Atualize os valores, datas de vencimento e quitação</p>
                    </div>
                </div>
            ...
            <form :action="editExpense.updateUrl" method="POST" class="p-5 space-y-4 overflow-y-auto flex-1">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="category" x-model="editExpense.category"
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0.01" name="amount" x-model="editExpense.amount" required
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" x-model="editExpense.notes" rows="2"
                              class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20 shadow-2xs"></textarea>
                </div>
