...
@section('page-title', 'Entradas da APM')

@section('header-actions')
    <a href="{{ route('admin.apm-incomes.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg">+ Nova Entrada</a>
@endsection
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden" x-data="adminTable()">
    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            @php $status = $income->status(); @endphp
            <tr class="hover:bg-gray-50"
                data-row="{{ strtolower($income->description . ' ' . ($income->category ?? '') . ' ' . $status) }}"
                data-active="1"
                data-desc="{{ strtolower($income->description) }}"
                data-cat="{{ strtolower($income->category ?? '') }}"
                data-status="{{ strtolower($status) }}">
                <td class="px-4 py-3 font-medium text-gray-800">{{ $income->description }}</td>
                <td class="px-4 py-3 text-gray-600">{{ $income->category ?? '—' }}</td>
                <td class="px-4 py-3 text-gray-600">R$ {{ number_format($income->amount, 2, ',', '.') }}</td>
                <td class="px-4 py-3 text-gray-500">{{ $income->due_date->format('d/m/Y') }}</td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center justify-end gap-1">
                        <form action="{{ route('admin.apm-incomes.mark-received', $income) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                            <button type="submit" title="{{ $income->received_date ? 'Marcar pendente' : 'Marcar recebida' }}"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg transition {{ $income->received_date ? 'bg-green-50 text-green-600 hover:bg-green-100' : 'bg-gray-100 text-gray-400 hover:bg-gray-200' }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </button>
                        </form>
                        <a href="{{ route('admin.apm-incomes.edit', $income) }}" title="Editar"
            ...
            @empty
            <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">Nenhuma entrada cadastrada ainda.</td></tr>
            @endforelse
        </tbody>
    </table>
