    ...
    <div class="w-full max-w-[1850px] mx-auto space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 text-xs font-semibold text-gray-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-indigo-600 font-semibold">Espelho Mensal</span>
                </div>
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                <button onclick="window.print()" class="inline-flex items-center gap-2 rounded-xl bg-white border border-gray-300 px-4 py-2.5 text-xs font-medium text-gray-700 shadow-2xs hover:bg-gray-50 transition">
                    <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                    <span>Imprimir Folha</span>
                </button>

                <a href="{{ route('admin.timeclock.index') }}" class="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-4 py-2.5 text-xs font-medium text-white shadow-xs transition hover:bg-gray-800">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
            <form method="GET" action="{{ route('admin.timeclock.mirror') }}" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div class="sm:col-span-2">
                    <label class="block text-[11px] font-medium text-gray-600 uppercase mb-1">Colaborador / Docente</label>
                ...
                <div>
                    <label class="block text-[11px] font-medium text-gray-600 uppercase mb-1">Mês de Competência</label>
                ...
                <div class="flex items-end">
                    <button type="submit" class="w-full rounded-xl bg-indigo-600 px-4 py-2 text-xs font-semibold text-white hover:bg-indigo-500 transition">
                        Consultar Espelho
                    </button>
                </div>
            </form>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                ...
                <div class="text-xl sm:text-2xl font-semibold tracking-tight text-emerald-700 font-mono mt-1">{{ $summary['total_worked_hours'] }}</div>
            </div>

            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                <span class="text-xs font-medium text-gray-500 block">Horas Programadas</span>
                <div class="text-xl sm:text-2xl font-semibold tracking-tight text-gray-900 font-mono mt-1">{{ $summary['total_expected_hours'] }}</div>
            </div>

            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                <span class="text-xs font-medium text-gray-500 block">Saldo de Horas</span>
                <div class="text-xl sm:text-2xl font-semibold tracking-tight font-mono mt-1 {{ $summary['balance_minutes'] >= 0 ? 'text-indigo-600' : 'text-rose-600' }}">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                ...
                <div class="text-xl sm:text-2xl font-semibold tracking-tight text-amber-600 font-mono mt-1">{{ $summary['total_delays_minutes'] }} min</div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-xs overflow-hidden">
            <div class="px-6 py-4 bg-gray-50/80 border-b border-gray-200 flex items-center justify-between">
                ...
                <span class="text-xs text-gray-500 font-normal">Portaria 671 MTE / Etec SAM</span>
            </div>
                        ...
                        <tr class="{{ $isWeekend ? 'bg-gray-50/50 text-gray-400' : 'hover:bg-gray-50/80' }} transition">
                            ...
                            <td class="px-3 py-3 text-center">
                                @if($d['punches']['entry_1'])
                                    <span class="font-medium text-emerald-700">{{ $d['punches']['entry_1']->recorded_at->format('H:i') }}</span>
                                @else
                            ...
                            <td class="px-3 py-3 text-center">
                                @if($d['punches']['exit_1'])
                                    <span class="font-medium text-amber-700">{{ $d['punches']['exit_1']->recorded_at->format('H:i') }}</span>
                                @else
                                    <span class="text-gray-300">-</span>
                                @endif
                            </td>
                            ...
                            <td class="px-3 py-3 text-center">
                                @if($d['punches']['entry_2'])
                                    <span class="font-medium text-emerald-700">{{ $d['punches']['entry_2']->recorded_at->format('H:i') }}</span>
                                @else
                                    <span class="text-gray-300">-</span>
                                @endif
                            </td>
                            ...
                            <td class="px-3 py-3 text-center">
                                @if($d['punches']['exit_2'])
                                    <span class="font-medium text-rose-700">{{ $d['punches']['exit_2']->recorded_at->format('H:i') }}</span>
                                @else
                                    <span class="text-gray-300">-</span>
                                @endif
                            </td>
                            ...
                            <!-- Worked Hours -->
                            <td class="px-4 py-3 text-center font-medium font-sans {{ $d['worked_minutes'] > 0 ? 'text-gray-900' : 'text-gray-400' }}">
                            ...
                            <td class="px-4 py-3 font-sans">
                                @if($d['medical_cert'])
                                    <span class="inline-flex items-center gap-1.5 rounded-md bg-blue-50 border border-blue-200 px-2 py-0.5 text-[11px] font-medium text-blue-700">
                                        <svg class="w-3 h-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-3-3v6m-7 4h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                        <span>Atestado Médico #{{ $d['medical_cert']->id }} (Justificado)</span>
                                    </span>
                                @elseif($d['legal_leave'])
                                    <span class="inline-flex items-center gap-1.5 rounded-md bg-purple-50 border border-purple-200 px-2 py-0.5 text-[11px] font-medium text-purple-700">
                                        <svg class="w-3 h-3 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3"/></svg>
                                        <span>Folga Prevista em Lei (Aprovada)</span>
                                    </span>
                                @elseif($d['delays'] > 0)
                                    <span class="inline-flex items-center gap-1 rounded-md bg-amber-50 text-amber-800 border border-amber-200 px-2 py-0.5 text-[11px] font-medium">
                                        ...
                                        <span>Atraso: +{{ $d['delays'] }}m</span>
                                    </span>
                                @elseif(!$hasPunches && $d['expected_minutes'] > 0)
                                    <span class="rounded-md bg-rose-50 text-rose-700 border border-rose-200 px-2 py-0.5 text-[11px] font-medium">
                                        Falta
                                    </span>
                                @else
