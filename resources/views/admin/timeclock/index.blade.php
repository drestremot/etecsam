    ...
    <div class="w-full max-w-[1850px] mx-auto space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                ...
                <a href="{{ route('admin.work-schedules.index') }}" class="inline-flex items-center gap-2 rounded-xl bg-white border border-gray-300 px-4 py-2.5 text-xs font-semibold text-gray-700 shadow-2xs hover:bg-gray-50 transition">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 sm:gap-4">
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                ...
                <div class="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 mt-1">{{ $stats['total_punches'] }}</div>
            </div>

            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                <span class="text-xs font-medium text-gray-500 block">Docentes Presentes</span>
                <div class="text-2xl sm:text-3xl font-bold tracking-tight text-indigo-700 mt-1">{{ $stats['distinct_users'] }}</div>
            </div>

            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                <span class="text-xs font-medium text-gray-500 block">Validadas Regularmente</span>
                <div class="text-2xl sm:text-3xl font-bold tracking-tight text-emerald-700 mt-1">{{ $stats['approved_punches'] }}</div>
            </div>

            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
                <span class="text-xs font-medium text-gray-500 block">Alertas de GPS (Fora)</span>
                <div class="text-2xl sm:text-3xl font-bold tracking-tight text-rose-600 mt-1">{{ $stats['flagged_outside'] }}</div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs col-span-2 sm:col-span-1">
                ...
                <div class="text-2xl sm:text-3xl font-bold tracking-tight text-amber-600 mt-1">{{ $stats['flagged_late'] }}</div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-xs">
            <form method="GET" action="{{ route('admin.timeclock.index') }}" class="grid grid-cols-1 sm:grid-cols-5 gap-3">
                <div>
                    <label class="block text-[11px] font-semibold text-gray-600 uppercase mb-1">Data</label>
                ...
                <div>
                    <label class="block text-[11px] font-semibold text-gray-600 uppercase mb-1">Professor / Colaborador</label>
                ...
                <div>
                    <label class="block text-[11px] font-semibold text-gray-600 uppercase mb-1">Status da Batida</label>
                    <select name="status" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-xs text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                        ...
                        <option value="flagged_outside_unit" {{ request('status') == 'flagged_outside_unit' ? 'selected' : '' }}>⚠️ Fora da Unidade (GPS)</option>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.timeclock.index') }}" class="rounded-xl border border-gray-300 px-3 py-2 text-xs font-semibold text-gray-600 hover:bg-gray-100 transition">
                        Limpar
                    </a>
                </div>
            </form>
                ...
                <table class="w-full text-left text-xs sm:text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50/80 transition">
                            ...
                            <td class="px-5 py-3.5">
                                @if($rec->photo_url)
                                    <button type="button" @click="openPhotoModal('{{ $rec->photo_url }}', '{{ $rec->user->name }}', '{{ $rec->recorded_at->format('d/m/Y H:i:s') }}')"
                                        ...
                                        <img src="{{ $rec->photo_url }}" alt="Snapshot" class="w-10 h-10 rounded-xl object-cover border-2 border-indigo-200 shadow-2xs group-hover:scale-105 transition">
                                        <span class="absolute -bottom-1 -right-1 bg-indigo-600 text-white rounded-full p-0.5 text-[9px]">🔍</span>
                                    </button>
                                @else
                            ...
                            <!-- Time & Type -->
                            <td class="px-5 py-3.5">
                                <div class="font-mono font-bold text-gray-900 text-xs sm:text-sm">
                                    {{ $rec->recorded_at->format('H:i:s') }}
                                </div>
                                <span class="inline-block mt-0.5 rounded-md px-2 py-0.5 text-[10.5px] font-bold border {{ $rec->getRecordTypeBadgeClass() }}">
                            ...
                            <!-- Employee -->
                            <td class="px-5 py-3.5">
                                <div class="font-bold text-gray-900">{{ $rec->user->name }}</div>
                                <div class="text-[11px] text-gray-500">{{ $rec->user->role ?? 'Colaborador' }}</div>
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-center">
                                @if($rec->distance_to_unit_meters !== null)
                                    <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10.5px] font-bold {{ $rec->is_within_geofence ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200' }}">
                                        {{ $rec->is_within_geofence ? '📍 Na Escola' : '⚠️ Fora' }} ({{ $rec->distance_to_unit_meters }}m)
                                    </span>
                                @elseif($rec->verification_method === 'totem_kiosk')
                                    <span class="rounded-full bg-blue-50 text-blue-700 border border-blue-200 px-2.5 py-0.5 text-[10.5px] font-bold">
                                        🖥️ Totem Físico
                                    </span>
                                @else
                            ...
                            <td class="px-5 py-3.5 text-center">
                                @if($rec->delay_minutes > 0)
                                    <span class="rounded-full bg-amber-100 text-amber-800 px-2.5 py-0.5 text-[10.5px] font-bold">
                                        +{{ $rec->delay_minutes }}m atraso
                                    </span>
                                @elseif($rec->is_within_schedule)
                                    <span class="text-emerald-600 font-bold text-xs">No Horário</span>
                                @else
                                    <span class="text-purple-600 font-semibold text-[11px]">Extra / Fora da Grade</span>
                                @endif
                            </td>

                            <!-- Status -->
                            <td class="px-5 py-3.5 text-center">
                                <span class="inline-block rounded-full px-2.5 py-1 text-xs font-bold border {{ $rec->getStatusBadgeClass() }}">
                                    {{ $rec->getStatusLabel() }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-right whitespace-nowrap">
                                @if($rec->status !== 'approved' && $rec->status !== 'justified')
                                    <button type="button" @click="openJustifyModal({{ $rec->id }}, '{{ $rec->user->name }}')"
                                            class="rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 px-3 py-1.5 text-xs font-bold transition">
                                        Justificar
                                    </button>
                                @elseif($rec->justification)
                                    <span class="text-[10px] text-gray-500 italic block truncate max-w-[120px]" title="{{ $rec->justification }}">
                                        "{{ $rec->justification }}"
                                    </span>
                                @else
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-gray-400">
                                ...
                                <p class="text-sm font-medium">Nenhum registro de ponto encontrado para esta data.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="photoModal.show" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex min-h-screen items-center justify-center p-4 text-center">
                ...
                <div class="relative transform overflow-hidden rounded-3xl bg-white p-6 text-center shadow-2xl transition-all sm:my-8 max-w-sm w-full space-y-4">
                    <h3 class="font-bold text-gray-900 text-base" x-text="photoModal.title"></h3>
                    <p class="text-xs text-gray-500" x-text="photoModal.subtitle"></p>
                    <img :src="photoModal.src" class="w-full aspect-square rounded-2xl object-cover border-2 border-indigo-100 shadow-md">
                    <button type="button" @click="photoModal.show = false" class="w-full rounded-xl bg-gray-900 py-2.5 text-xs font-bold text-white hover:bg-gray-800">
                        Fechar
                    </button>
                </div>
            </div>
        ...
        <div x-show="justifyModal.show" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex min-h-screen items-center justify-center p-4 text-center">
                ...
                <div class="relative transform overflow-hidden rounded-3xl bg-white p-6 text-left shadow-2xl transition-all sm:my-8 max-w-md w-full space-y-4">
                    <h3 class="font-bold text-gray-900 text-base">Justificar Ocorrência de Ponto</h3>
                    <p class="text-xs text-gray-500" x-text="'Colaborador: ' + justifyModal.userName"></p>

                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Motivo / Despacho da Direção *</label>
                        <textarea x-model="justifyModal.text" rows="3" placeholder="Ex: Problema técnico de GPS / Atividade pedagógica externa autorizada..."
                                  class="w-full rounded-2xl border border-gray-300 p-3 text-xs text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-400"></textarea>
                    </div>

                    <div class="flex justify-end gap-2">
                        <button type="button" @click="justifyModal.show = false" class="rounded-xl border border-gray-300 px-4 py-2 text-xs font-bold text-gray-700 hover:bg-gray-50">
                            Cancelar
                        </button>
                        <button type="button" @click="submitJustification()" class="rounded-xl bg-blue-600 px-5 py-2 text-xs font-bold text-white hover:bg-blue-500">
