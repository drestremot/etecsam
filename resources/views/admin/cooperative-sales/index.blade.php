...
@section('page-title', 'Vendas da Cooperativa')

@section('header-actions')
    <a href="{{ route('admin.cooperative-sales.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2 rounded-lg">+ Nova Venda</a>
@endsection
    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-50">
            <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Data da Venda</th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50"
                data-row="{{ strtolower($sale->description . ' ' . ($sale->category ?? '') . ' ' . $status) }}"
                data-active="1"
                data-desc="{{ strtolower($sale->description) }}"
                data-cat="{{ strtolower($sale->category ?? '') }}"
                data-status="{{ strtolower($status) }}">
                <td class="px-4 py-3 font-medium text-gray-800">{{ $sale->description }}</td>
                <td class="px-4 py-3 text-gray-600">{{ $sale->category ?? '—' }}</td>
                <td class="px-4 py-3 text-gray-600">R$ {{ number_format($sale->amount, 2, ',', '.') }}</td>
                <td class="px-4 py-3 text-gray-500">{{ $sale->sale_date->format('d/m/Y') }}</td>
                <td class="px-4 py-3">
                    <span class="px-2 py-1 rounded-full text-xs font-semibold
                        {{ $status === 'Recebido' ? 'bg-green-100 text-green-700' : ($status === 'Atrasado' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700') }}">
                        {{ $status }}
                    </span>
                </td>
                <td class="px-4 py-3">
                    <div class="flex items-center justify-end gap-1">
                        <form action="{{ route('admin.cooperative-sales.mark-received', $sale) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                            <button type="submit" title="{{ $sale->received_date ? 'Marcar pendente' : 'Marcar recebido' }}"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg transition {{ $sale->received_date ? 'bg-green-50 text-green-600 hover:bg-green-100' : 'bg-gray-100 text-gray-400 hover:bg-gray-200' }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </button>
                        </form>
                        <a href="{{ route('admin.cooperative-sales.edit', $sale) }}" title="Editar"
            ...
            @empty
            <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">Nenhuma venda cadastrada ainda.</td></tr>
            @endforelse
        </tbody>
    </table>
