    ...
    <div class="w-full max-w-4xl mx-auto space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 text-xs font-semibold text-gray-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-teal-700 font-bold">Valores de Mensalidade</span>
                </div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-3">
                    <span class="flex items-center gap-2">
                        ...
                        Competências e Valores das Mensalidades
                    </span>
                </h1>
                <p class="text-xs text-gray-600 mt-0.5 font-normal">
                    Definição do valor cobrado por mês aos estudantes cooperados
                </p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('admin.cooperative-members.index') }}" class="inline-flex items-center gap-1.5 rounded-xl bg-teal-700 px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-teal-600">
        ...
        <div x-show="createOpen"
            ...
            <div class="flex items-center justify-between border-b border-gray-100 pb-3 mb-4">
                <h2 class="text-sm font-bold text-gray-800">Cadastrar Nova Competência / Valor de Mensalidade</h2>
            ...
            <form action="{{ route('admin.cooperative-monthly-fees.store') }}" method="POST" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">
                            Valor da Mensalidade (R$) <span class="text-rose-500">*</span>
                        </label>
                        <input type="number" step="0.01" min="0" name="amount" value="{{ old('amount', '20.00') }}" required
                               class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs text-gray-800 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 shadow-2xs">
                    </div>
                </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5 shadow-xs space-y-4">
            <div class="overflow-x-auto rounded-xl border border-gray-200">
                <table class="min-w-full divide-y divide-gray-200 text-left text-xs text-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse($cooperativeMonthlyFees as $fee)
                            <tr class="hover:bg-teal-50/30 transition">
                                ...
                                <td class="px-4 py-3 font-bold text-teal-700 text-sm">
                                    R$ {{ number_format($fee->amount, 2, ',', '.') }}
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex items-center gap-1">
                                        <button @click="openEdit({{ json_encode($fee) }}, '{{ route('admin.cooperative-monthly-fees.update', $fee) }}')"
                                                type="button"
                                                title="Editar"
                                        ...
                                        <form action="{{ route('admin.cooperative-monthly-fees.destroy', $fee) }}" method="POST" class="inline"
            ...
            @if($cooperativeMonthlyFees->hasPages())
                <div class="pt-2">
                    {{ $cooperativeMonthlyFees->links() }}
                </div>
            @endif
        </div>
    ...
    <div x-show="editOpen"
        ...
        <div @click.outside="editOpen = false"
            ...
            <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                <h2 class="text-sm font-bold text-gray-900">Editar Valor da Mensalidade</h2>
