...
@section('page-title', $action === 'create' ? 'Novo Documento da APM' : 'Editar Documento')

@section('content')
<div class="max-w-2xl">

    <div class="mb-4">
        <a href="{{ route('admin.apm-reports.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-indigo-600 transition">
            ← Voltar para lista
        </a>
    </div>

    <form action="{{ $action === 'create' ? route('admin.apm-reports.store') : route('admin.apm-reports.update', $apmReport) }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $apmReport->title) }}" required
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        @foreach(['Estatuto', 'Ata de Reunião', 'Prestação de Contas'] as $cat)
                        <label class="flex items-center gap-2 p-3 rounded-lg border-2 cursor-pointer transition text-sm
                            {{ old('category', $apmReport->category) === $cat ? 'border-indigo-500 bg-indigo-50 text-indigo-700 font-semibold' : 'border-gray-200 hover:border-gray-300 text-gray-600' }}">
                            <input type="radio" name="category" value="{{ $cat }}"
                                   {{ old('category', $apmReport->category) === $cat ? 'checked' : '' }}
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="period" value="{{ old('period', $apmReport->period) }}"
                               class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                    ...
                    <div>
                        ...
                        <input type="date" name="published_at"
                               value="{{ old('published_at', $apmReport->published_at ? \Carbon\Carbon::parse($apmReport->published_at)->format('Y-m-d') : '') }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    @if($action === 'edit' && !empty($apmReport->file_path))
                        <div class="flex items-center gap-3 mb-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <span class="text-2xl"><svg class="w-4 h-4 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg></span>
                            <div>
                                <p class="text-sm font-medium text-gray-700">Arquivo atual</p>
                                <a href="{{ photo_url($apmReport->file_path) }}" target="_blank"
                ...
                <div>
                    ...
                    <input type="url" name="url" value="{{ old('url', $apmReport->url) }}"
                           class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
