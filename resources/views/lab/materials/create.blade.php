...
<div class="min-h-screen bg-[#dfe1e5] px-3 sm:px-6 py-4 sm:py-6 pb-20 sm:pb-6">
    <div class="w-full max-w-2xl mx-auto space-y-5">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-gray-800 uppercase">Novo Material</h1>
                <p class="text-xs text-gray-500">Cadastre um novo item ou insumo para o inventário</p>
            </div>
        </div>

        <form action="{{ route('lab.materials.store') }}" method="POST" enctype="multipart/form-data" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
            @csrf

            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase mb-1.5">Nome do Material / Equipamento *</label>
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="Ex: Microscópio Óptico Binocular"
            ...
            <div>
                ...
                <textarea name="description" rows="2" placeholder="Especificações técnicas, marca, modelo, estado..."
                          class="w-full rounded-xl border border-gray-300 bg-gray-50/50 px-3.5 py-2.5 text-xs text-gray-800 shadow-2xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="unit" value="{{ old('unit', 'unid.') }}" placeholder="Ex: unid, kit, caixa, par"
                           class="w-full rounded-xl border border-gray-300 bg-gray-50/50 px-3.5 py-2.5 text-xs text-gray-800 shadow-2xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-1.5">Nº de Patrimônio</label>
                    <input type="text" name="patrimony_number" value="{{ old('patrimony_number') }}" placeholder="Ex: PAT-2026-004"
                           class="w-full rounded-xl border border-gray-300 bg-gray-50/50 px-3.5 py-2.5 text-xs text-gray-800 shadow-2xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500">
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-100">
                ...
                <button type="submit" class="rounded-xl bg-purple-600 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-purple-500 transition">Salvar Material</button>
            </div>
        </form>
    </div>
</div>
