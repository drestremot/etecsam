                ...
                <table class="w-full text-left text-xs sm:text-sm">
                    <thead class="bg-gray-50 text-xs font-semibold uppercase text-gray-500 border-b border-gray-200 tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-4">Papel no Sistema (Alteração Rápida)</th>
                            <th class="px-6 py-4">Vínculos de Coordenação</th>
                        ...
                        <tr class="hover:bg-gray-50/80 transition {{ !$u->is_active ? 'opacity-60' : '' }}"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center text-xs font-bold flex-shrink-0 shadow-2xs">
                                        {{ strtoupper(substr($u->name, 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="font-bold text-gray-900 truncate text-xs sm:text-sm">{{ $u->name }}</p>
                                        <p class="text-xs text-gray-500 truncate">{{ $u->role ?? ($u->roles->first()?->name ?? 'Colaborador') }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-gray-700 font-mono text-xs">{{ $u->email }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <form action="{{ route('lab.users.status', $u) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <button class="px-3 py-1 rounded-full text-xs font-semibold transition shadow-2xs {{ $u->is_active ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' : 'bg-red-100 text-red-800 hover:bg-red-200' }}">
